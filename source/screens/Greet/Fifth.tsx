...
const GreetFifth: React.FC = () => {
  ...
  const onNext = () => {
    nav.navigate(Routes.Greet_Sixth);
  };
      ...
      <View className=" mt-3">
        <View className="self-start">
          ...
          <Text className="mt-3.5 text-lg text-white" accessibilityRole="text">
            Add all your income and outcomes
          </Text>
        </View>
      </View>

      <Image
        source={require('@/media/images/greet/5.png')}
      ...
      <View className="flex-row justify-between mt-auto  mb-4">
        ...
        <View className="items-center justify-center gap-4">
          <View className="flex-row gap-1">
            {dots.map((_, index) => (
              <DotIndicator key={index} isActive={index === 3} />
            ))}
          </View>
          <TouchableOpacity
            onPress={onSkip}
